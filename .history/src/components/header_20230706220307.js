        ...
        <div className='container mx-auto p-4'>
            <div className='flex justify-between items-center'>
                ...
                <div className='text-right'>
                    <div onClick={toggleMenu} className={`${isOpen ? 'text-black -top-4 -right-4' : ''} text-white p-2 text-3xl hidden max-sm:inline-block relative z-40`}>{isOpen ? <LiaTimesSolid /> : <RxHamburgerMenu />}</div>
                    <ul className={`${isOpen ? '' : 'max-sm:translate-x-full'} sm:flex sm:items-center max-sm:text-left gap-10 max-sm:fixed max-sm:top-0 max-sm:right-0 max-sm:bottom-0 max-sm:bg-white max-sm:w-2/3 max-sm:gap-2 max-sm:p-6 max-sm:h-full max-sm:transition`}>
                        <li 
